...
const ForeGround: React.FC<ForeGroundProps> = memo(() => {
  const {currentProfile:{fname},activeUser:{profileOwner}} = useProfile(); 
    ...
    <View style={{flex: 1,marginTop: Platform.OS === 'android' ? -120 : -50 }}>
      ...
      <View style={{flex:1,padding:5,flexDirection:'row'}}>
        ...
        <View style={{marginLeft:30}}>
            {profileOwner ? (
                <TouchableOpacity onPress={() => handleUploadPhotos('avatar','KID')} style={{backgroundColor:colors.primary,padding:5,borderRadius:100,height:48,width:48,alignItems:'center',justifyContent:'center'}}>
